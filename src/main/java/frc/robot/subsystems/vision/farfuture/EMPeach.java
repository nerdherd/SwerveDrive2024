...
public class EMPeach implements Reportable{
    ...
    public double[] getMovePowerToImp(double metersAwayFromTarget) {
        ...
        toggleEMP(false);
        changeEMPType(3);
        if(!limelight.hasValidTarget()) return powers;

        double range;
        ranges[rangeIndex] = getDistanceFromImp();
        rangeIndex++;
        ...
        if(initDone) {
            ...
            range = rangeSum / ranges.length;
        }
        else {
            for (int i = 0; i < rangeIndex; i++) {
                rangeSum += ranges[i];
            }
            range = rangeSum / rangeIndex;
        }
    ...
    public void initShuffleboard(LOG_LEVEL priority) {
        ...
        switch (priority) {
            ...
            case MINIMAL:   
                // tab.addCamera(limelightName + ": Stream", limelightName, VisionConstants.kLimelightIP + ":5800")
                //     .withPosition(0, 0)
                //     .withSize(6, 3);
